...
from xjtlu_uim_login import credentials, load_project_env, login as uim_login, otp_now
...
def login_owa(email_account: str, uim_cookies: list[dict] | None = None):
    """
    完成 UIM + OWA SSO。
    返回 (requests.Session, canary, tgc)。
    """
    ...
    for _ in range(12):
        ...
        if canary:
            print(f"[+] 成功登录 OWA，Canary: {canary[:10]}...")
